...
def get_cell_and_no_cell_patches_from_video(video_filename,
    ...
    non_cell_patches = np.zeros_like(frames, shape=[3 * n_cells_in_vid, *patch_size])
...
def get_cell_and_no_cell_patches(patch_size=(21, 21), n_negatives_per_positive=3, do_hist_match=False):
    ...
    trainset_filename = os.path.join(
        ...
        f'trainset_bloodcells_ps_{patch_size[0]}_hist_matched_{str(do_hist_match).lower()}_npp_{n_negatives_per_positive}.pt')
    validset_filename = os.path.join(
        CACHED_DATASETS_FOLDER,
        f'validset_bloodcells_ps_{patch_size[0]}_hist_matched_{str(do_hist_match).lower()}_npp_{n_negatives_per_positive}.pt')

    cell_images_filename = os.path.join(
        CACHED_DATASETS_FOLDER,
        f'bloodcells_ps_{patch_size[0]}_hist_matched_{str(do_hist_match).lower()}_npp_{n_negatives_per_positive}.npy')
    non_cell_images_filename = os.path.join(
        CACHED_DATASETS_FOLDER,
        f'non_bloodcells_ps_{patch_size[0]}_hist_matched_{str(do_hist_match).lower()}_npp_{n_negatives_per_positive}.npy')
    cell_images_marked_filename = os.path.join(
        CACHED_DATASETS_FOLDER,
        f'bloodcells_ps_{patch_size[0]}_hist_matched_{str(do_hist_match).lower()}_npp_{n_negatives_per_positive}_marked.npy')
    non_cell_images_marked_filename = os.path.join(
        CACHED_DATASETS_FOLDER,
        f'non_bloodcells_ps_{patch_size[0]}_hist_matched_{str(do_hist_match).lower()}_npp_{n_negatives_per_positive}_marked.npy')
