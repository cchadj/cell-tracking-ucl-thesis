...
all_video_filenames = [os.path.join(SHARED_VIDEOS_PATH, f) for f in os.listdir(SHARED_VIDEOS_PATH) if
                       f.endswith('.avi')]
...
all_video_filenames.extend([
    os.path.join(REGISTERED_VIDEOS_2_MARKED_FOLDER, f) for f in os.listdir(REGISTERED_VIDEOS_2_MARKED_FOLDER) if
    f.endswith('.avi')
])

all_video_filenames_unmarked = [f for f in all_video_filenames if 'marked' not in basename(f) and 'mask' not in basename(f)]
unmarked_video_OA790_filenames = [f for f in all_video_filenames_unmarked if 'OA790' in basename(f)]
unmarked_video_OA850_filenames = [f for f in all_video_filenames_unmarked if 'OA850' in basename(f)]

all_video_filenames_marked = [f for f in all_video_filenames if 'marked' in basename(f) and 'mask' not in basename(f)]
marked_video_OA790_filenames = [f for f in all_video_filenames_marked if 'OA790' in basename(f)]
marked_video_OA850_filenames = [f for f in all_video_filenames_marked if 'OA850' in basename(f)]

csv_cell_cords_filenames = [os.path.join(SHARED_VIDEOS_PATH, f) for f in os.listdir(SHARED_VIDEOS_PATH) if
                            f.endswith('.csv')]
...
_unmarked_video_OA790_filenames_all = [
    f for f in all_video_filenames if
    'OA790nm' in basename(f) and 'marked' not in basename(f) and 'mask' not in basename(f)
]
...
_unmarked_video_OA850_filenames_all = [
    f for f in all_video_filenames if
    'marked' not in basename(f) and 'OA850nm' in basename(f) and 'mask' not in basename(f)
]
...
video_file_OA790_dictionaries = []
for video_filename in unmarked_video_OA790_filenames:
    video_type = ''
    if 'Confocal' in video_filename:
        video_type = 'confocal'
    elif 'OA790' in video_filename:
        video_type = 'OA790'
    elif 'OA850' in video_filename:
        video_type = 'OA790'
    ...
    vid_marked_790_filename = find_filename_of_same_source(video_filename, marked_video_OA790_filenames)
    vid_marked_850_filename = find_filename_of_same_source(video_filename, marked_video_OA850_filenames)
    is_marked = vid_marked_790_filename != '' or vid_marked_850_filename != ''
    video_file_dict = {
        'type': video_type,
        'is_marked': is_marked,
        ...
        'video_790_filename': find_filename_of_same_source(video_filename, unmarked_video_OA790_filenames),
        'video_850_filename': find_filename_of_same_source(video_filename, unmarked_video_OA850_filenames),
        'video_marked_790_filename': vid_marked_790_filename,
        'video_marked_850_filename': vid_marked_850_filename,
        'mask_video_filename': find_filename_of_same_source(video_filename, mask_video_filenames),
        'vessel_mask_image_filename': None,
        'std_confocal_image_filename': find_filename_of_same_source(video_filename, std_images_confocal),
        'std_790_image_filename': find_filename_of_same_source(video_filename, std_images_OA790),
        'std_850_image_filename': find_filename_of_same_source(video_filename, std_images_OA850),
        'coordinate_csv_filenames': [csv_file for csv_file in
                                     csv_cell_cords_OA790_filenames if
                                     files_of_same_source(csv_file, video_filename)]}
    video_file_OA790_dictionaries.append(video_file_dict)

if __name__ == '__main__':
    assert len(unmarked_labeled_video_OA790_filenames) == len(marked_video_OA790_filenames)
    for i, (unmarked_video, marked_video) in enumerate(
            zip(unmarked_labeled_video_OA790_filenames, marked_video_OA790_filenames)):
    ...
    for i, (unmarked_video, csv) in enumerate(
            zip(unmarked_labeled_video_OA790_filenames, csv_cell_cords_OA790_filenames)):
